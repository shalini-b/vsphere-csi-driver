...
func newReconciler(mgr manager.Manager, clusterFlavor cnstypes.CnsClusterFlavor,
	configInfo *cnsconfig.ConfigurationInfo, recorder record.EventRecorder) reconcile.Reconciler {
...
func updateCRStatus(ctx context.Context, r *ReconcileCSINodeTopology, instance *csinodetopologyv1alpha1.CSINodeTopology,
	status csinodetopologyv1alpha1.CRDStatus, failureEventMessage string) error {
	log := logger.GetLogger(ctx)
	...
	case csinodetopologyv1alpha1.CSINodeTopologySuccess:
		...
		instance.Status.ErrorMessage = failureEventMessage
		// Record an event on the CR
		r.recorder.Event(instance, corev1.EventTypeNormal, "CSINodeTopologySucceeded",
			fmt.Sprintf("Topology labels updated for nodeVM %q", instance.Spec.NodeID))
	case csinodetopologyv1alpha1.CSINodeTopologyError:
		instance.Status.ErrorMessage = failureEventMessage
		// Record an event on the CR
		r.recorder.Event(instance, corev1.EventTypeWarning, "CSINodeTopologyFailed", failureEventMessage)
	}
