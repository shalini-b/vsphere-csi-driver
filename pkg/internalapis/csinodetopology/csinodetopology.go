...
func topologyCRAdded(obj interface{}) {
	...
	nodeTopology, ok := obj.(*csinodetopologyv1alpha1.CSINodeTopology)
	if !ok {
		log.Warnf("topologyCRAdded: unrecognized object %+v", obj)
		return
	}
	updateNodeTopologyInfo(*nodeTopology)
}

func topologyCRUpdated(oldObj, newObj interface{}) {
	_, log := logger.GetNewContextWithLogger()
	// Validate the old object received.
	oldNodeTopology, ok := oldObj.(*csinodetopologyv1alpha1.CSINodeTopology)
	if !ok {
		log.Warnf("topologyCRUpdated: Unrecognized old object %+v", oldObj)
		return
	}
	// Validate the new object received.
	newNodeTopology, ok := newObj.(*csinodetopologyv1alpha1.CSINodeTopology)
	if !ok {
		log.Warnf("topologyCRUpdated: Unrecognized new object %+v", newObj)
		return
	}
	// Check if there is any update in topology labels, if not return.
	...
	updateNodeTopologyInfo(*newNodeTopology)
}
...
func updateNodeTopologyInfo(nodeTopology csinodetopologyv1alpha1.CSINodeTopology) {
	...
	if nodeTopology.Status.Status == csinodetopologyv1alpha1.CSINodeTopologySuccess {
		...
		for _, topoLabel := range nodeTopology.Status.TopologyLabels {
			topologyLabels[topoLabel.Key] = topologyLabels[topoLabel.Value]
		}
...
func (volTopology *volumeTopology) getSharedDatastoresInTopology(ctx context.Context, topologyArr []*csi.Topology) (
	...
	for _, topology := range topologyArr {
		...
		log.Debugf("Obtained list of nodeVMs [%+v]", nodeVMsInZoneRegion)
		...
		log.Debugf("Obtained shared datastores: %+v", sharedDatastores)
