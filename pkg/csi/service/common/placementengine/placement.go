...
func GetSharedDatastores(ctx context.Context, topologySegmentsList []map[string]string, storagePolicyID, vcHost string,
	...
	for _, segments := range topologySegmentsList {
		...
		if err != nil {
			if err == cnsvsphere.ErrNoSharedDatastoresFound {
				log.Errorf("no shared datastores found among nodeVMs: %+v", matchingNodeVMs)
				continue
			}
		...
		if storagePolicyID != "" {
			...
			if len(compatibleDatastores) == 0 {
				log.Errorf("No compatible shared datastores found for storage policy %q", storagePolicyID)
				continue
			}
		...
		if isTopologyPreferentialDatastoresFSSEnabled && len(preferredDatastoresMap[vcHost]) != 0 {
			...
			for _, topoSegs := range completeTopologySegments {
				prefDS := GetPreferredDatastoresInSegments(ctx, topoSegs, vcHost)
